...
class Model(model.MLDoc.base.Model):
    ...
    def daeyong_cross(self, x, disable=False):
        ...
        if not disable and self.training:
            ...
            if x in self.worddict.src2tgt[lan]: 
                ...
                if (x in self.posdict) and (self.posdict[x] == "ADJ"): # if word is ADJ, ADV, V
                    return self.worddict.src2tgt[lan][x][random.randint(0,len(self.worddict.src2tgt[lan][x]) - 1)]
    ...
    def daeyong_cross_list(self, x, disable=False):
        return [self.daeyong_cross_str(xx, not (self.training and self.args.train.ratio >= random.random())) for xx in x]
    ...
    def forward(self, batch):
        ...
        _, utt = self.bert(*util.convert.List.to_bert_info(self.daeyong_cross_list(util.tool.in_each(batch, lambda x : x[0])), self.tokener, self.pad, self.cls, self.device, 128)[0])
        out = self.P(utt)
        loss = torch.Tensor([0])
        if self.training:
    ...
    def start(self, inputs):
        ...
        if not self.args.model.test: # Dir for SC
