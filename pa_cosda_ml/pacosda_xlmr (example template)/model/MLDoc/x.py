...
class Model(model.MLDoc.base.Model):
    def __init__(self, args, DatasetTool, inputs):
        ...
        #
        _, _, _, ontology, worddict, _, posdict = inputs  # daeyong
        self.ontology = ontology
        self.worddict = worddict
        self.posdict = posdict  # daeyong
        BERTTool.init(self.args)
        #
    ...
    def set_optimizer(self):
        ...
        if self.args.train.bert == False:
            ...
            params = [
                {"params": list(all_params - bert_params), "lr": self.args.lr.default}
            ]
        else:
            bert_params = set(BERTTool.multi_bert.parameters())
            params = [
    ...
    def run_eval(self, train, dev, test):
        ...
        for set_name, dataset in ds.items():
            ...
            summary.update(
                {"eval_{}_{}".format(set_name, k): v for k, v in tmp_summary.items()}
            )
        ...
        for epoch in range(self.args.train.epoch):
            ...
            if not self.args.train.not_eval:
                ...
                for set_name, dataset in ds.items():
                    ...
                    summary.update(
                        {
                            "eval_{}_{}".format(set_name, k): v
                            for k, v in tmp_summary.items()
                        }
                    )
    ...
    def cross(self, x, disable=False):
        if not disable and self.training and (self.args.train.cross >= random.random()):
            ...
            if x in self.worddict.src2tgt[lan]:
                return self.worddict.src2tgt[lan][x][
                    random.randint(0, len(self.worddict.src2tgt[lan][x]) - 1)
                ]
            else:
                return x
        else:
    ...
    def cross_list(self, x, disable=False):
        return [
            self.cross_str(
                xx, not (self.training and self.args.train.ratio >= random.random())
            )
            for xx in x
        ]
    ...
    # def cross(self, x, disable=False):
    #     if not disable and self.training and (self.args.train.cross >= random.random()):
    #         # daeyong / If ramdom() is above threshold and it's in training process
    #         lan = random.randint(0,len(self.args.dict_list) - 1)
    #         # daeyong / Then select one language from dict_list
    #         if x in self.worddict.src2tgt[lan]:
    #             # daeyong / if the word exists in the dictionary
    #             # daeyong / then return the word
    ...
    def daeyong_cross(self, x, disable=False):

        # cand = {"ADJ", "ADV", "V"}

        if not disable and self.training:
            # print("\n\n"+x+"\n\n")
            # print(self.posdict["the"])
            lan = random.randint(0, len(self.args.dict_list) - 1)
            if x in self.worddict.src2tgt[lan]:
                # if (x in self.posdict) and (self.posdict[x] in cand): # if word is ADJ, ADV, V
                if (x in self.posdict) and (
                    self.posdict[x] == "ADJ"
                ):  # if word is ADJ, ADV, V
                    return self.worddict.src2tgt[lan][x][
                        random.randint(0, len(self.worddict.src2tgt[lan][x]) - 1)
                    ]
                else:  # if word is not ADJ, ADV, V
                    if self.args.train.cross >= random.random():
                        return self.worddict.src2tgt[lan][x][
                            random.randint(0, len(self.worddict.src2tgt[lan][x]) - 1)
                        ]
        return x

    def daeyong_cross_str(self, x, disable=False):
        raw = x.lower().split(" ")
        out = ""
        for xx in raw:
            out += self.daeyong_cross(xx, disable)
            out += " "
        # print(out) # original
        return out

    def daeyong_cross_list(self, x, disable=False):
        return [
            self.daeyong_cross_str(
                xx, not (self.training and self.args.train.ratio >= random.random())
            )
            for xx in x
        ]
    ...
    def forward(self, batch):
        ...
        a, b, c = util.convert.List.to_bert_info(
            self.daeyong_cross_list(util.tool.in_each(batch, lambda x: x[0])),
            self.tokener,
            self.pad,
            self.cls,
            self.device,
    ...
    def start(self, inputs):
        ...
        train, dev, test, _, _, _, posdict = inputs  # daeyong
    ...
    def load_w(self, file):
        ...
        new_state = {
            "bias": torch.zeros(self.args.dimension.emb),
            "weight": state["weight"],
        }
