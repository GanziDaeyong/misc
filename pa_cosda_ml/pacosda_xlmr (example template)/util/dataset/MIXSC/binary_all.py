...
class DatasetTool(object):
    ...
    ## daeyong ##
    def get_pos_dict(file, args):
        raw = util.data.Delexicalizer.remove_linefeed(util.data.Reader.read_raw(file))
        pos_dict = {}
        for line in raw:
            token, pos = line.split("\t")
            pos_dict[token] = pos.strip()
        return pos_dict
    ## daeyong ##
    ...
    def get(args):
        ...
        # daeyong #
        pos_file = os.path.join(args.dir.dataset, args.dataset.pos)
        pos_dict = DatasetTool.get_pos_dict(pos_file, args)
        # daeyong #
        
        #return train, dev, test, None, idx_dict, None
        return train, dev, test, None, idx_dict, None, pos_dict
